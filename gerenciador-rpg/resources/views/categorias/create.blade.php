<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Criar Nova Pasta</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('categorias.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="descricao" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    </div>
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Salvar Pasta</button>
                    <a href="{{ route('fichas.index') }}" class="ml-2 text-gray-600 hover:underline">Cancelar</a>
                </form>
            </div>
        </div>
    </div>
